...
export default function NoticePopup({ notices, onClose }: NoticePopupProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-200 dark:bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-slate-50 dark:bg-zinc-900 border border-purple-500/30 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="bg-purple-600/20 p-4 flex items-start gap-3 border-b border-purple-500/20">
          <div className="p-2 bg-purple-500/20 rounded-full text-purple-400">
            <Bell className="w-5 h-5 animate-pulse" />
          </div>
          <div className="flex-1">
            <h3 className="font-bold text-slate-900 dark:text-white tracking-wide">{currentNotice.title || "Admin Notice"}</h3>
        ...
        <div className="p-5">
          <p className="text-sm text-slate-800 dark:text-zinc-200 leading-relaxed whitespace-pre-wrap max-h-60 overflow-y-auto pr-2 custom-scrollbar">
            {currentNotice.content}
          </p>
        </div>

        <div className="p-4 bg-slate-100 dark:bg-zinc-950/50 flex justify-end">
          <button
            onClick={handleNext}
            className="px-6 py-2.5 bg-purple-600 hover:bg-purple-500 text-slate-900 dark:text-white text-sm font-bold rounded-xl transition-transform active:scale-95"
          >
