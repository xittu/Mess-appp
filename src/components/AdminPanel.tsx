...
import { X, Search, FileText, Activity, Users, Download, ArrowLeft, Home } from "lucide-react";
...
export default function AdminPanel({ onClose }: { onClose: () => void }) {
  ...
  const handleDownloadPDF = (messData: MessData) => {
    const doc = new jsPDF();
    const M_Name = messData.mess_name || messData.meals?.messName || "Unknown Mess";
    
    // Title
    doc.text(`Mess Dashboard Report: ${M_Name}`, 14, 15);
    doc.setFontSize(10);
    doc.text(`Admin Email: ${messData.user_email}`, 14, 22);
    doc.text(`Last Updated: ${new Date(messData.last_updated).toLocaleString()}`, 14, 28);
  ...
  const filteredData = data.filter(d => 
    (d.user_email && d.user_email.toLowerCase().includes(search.toLowerCase())) ||
    (d.mess_name && d.mess_name.toLowerCase().includes(search.toLowerCase())) ||
    (d.meals?.messName && d.meals.messName.toLowerCase().includes(search.toLowerCase()))
  );
    ...
    <div className="fixed inset-0 z-50 bg-[#0F0C15] flex flex-col font-sans overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-purple-950/40 bg-zinc-950/80">
        <div className="flex items-center gap-2">
           {selectedMess ? (
             <button onClick={() => setSelectedMess(null)} className="p-1 rounded bg-zinc-800 hover:bg-zinc-700 text-zinc-300">
           ...
           <h2 className="text-sm font-bold text-white tracking-wide">
             {selectedMess ? `Mess Details: ${selectedMess.mess_name || selectedMess.meals?.messName || "Unknown"}` : "Admin Data Monitor"}
           </h2>
        </div>
        <button
          onClick={onClose}
      ...
      {loading ? (
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 text-zinc-200">
           ...
           <div className="bg-zinc-900 border border-purple-900/30 p-4 rounded-xl">
             ...
             <div className="text-sm font-bold text-white mb-3">{selectedMess.user_email}</div>
             
             <div className="text-xs text-zinc-400 mb-1">Total Members</div>
             <div className="text-sm font-bold text-white mb-3">{selectedMess.members?.length || 0} Members</div>

             <div className="text-xs text-zinc-400 mb-1">Total Market Expenses</div>
             <div className="text-sm font-bold text-emerald-400 mb-3">
               ৳{selectedMess.expenses?.bazaar?.reduce((sum, e) => sum + e.amount, 0) || 0}
             </div>

             <div className="text-xs text-zinc-400 mb-1">Fixed Meal Count</div>
             <div className="text-sm font-bold text-brand-amber mb-3">{selectedMess.meals?.fixedMealCount || 0} Meals</div>
        ...
        <div className="flex-1 flex flex-col p-4">
           ...
           <div className="flex-1 overflow-y-auto space-y-3 pb-6">
              {filteredData.map((d, i) => {
                 ...
                 return (
                   <div key={i} className="bg-zinc-900 border border-zinc-800 p-3 rounded-xl flex items-center justify-between">
                      <div>
                        <div className="font-bold text-sm text-white flex items-center gap-1.5">
                           <Home className="w-3.5 h-3.5 text-brand-accent" />
                           {MName}
                        </div>
                        <div className="text-[11px] text-zinc-400 mt-0.5">{d.user_email}</div>
                        <div className="text-[10px] text-zinc-500 mt-1 flex items-center gap-2">
                           <span className="flex items-center gap-1"><Users className="w-3 h-3" /> {d.members?.length || 0}</span>
                           <span>| Updated: {new Date(d.last_updated).toLocaleDateString()}</span>
                        </div>
                      </div>
                      <button 
              ...
              {filteredData.length === 0 && (
                 <div className="text-center text-sm text-zinc-500 py-10">No records found.</div>
              )}
           </div>
        </div>
      )}
