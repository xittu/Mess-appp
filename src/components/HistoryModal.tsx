    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-[#0F0C15] w-full max-w-2xl rounded-2xl border border-purple-900/40 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-4 border-b border-purple-900/30 flex items-center justify-between bg-zinc-950/80">
          ...
          <button onClick={onClose} className="p-1 rounded bg-zinc-900 text-zinc-400 hover:text-white">
            <X className="w-5 h-5" />
          </button>
        </div>
         ...
         <div className="p-4 flex-1 overflow-y-auto">
           ...
           <div className="flex flex-col gap-4 mb-6">
             <div className="flex items-center gap-3 bg-zinc-900/50 p-4 rounded-xl border border-zinc-800">
               ...
               <div className="flex-1">
                 <h3 className="text-zinc-200 font-bold font-sans">লাস্ট ৩ মাসের রিপোর্ট</h3>
             ...
             <div className="grid grid-cols-1 gap-3">
               {[0, 1, 2].map((monthOffset) => {
                 ...
                 return (
                   <div key={monthOffset} className="flex items-center justify-between bg-zinc-950 p-4 rounded-xl border border-zinc-800/80">
                     <span className="text-emerald-100/90 font-bold font-sans text-sm">{monthName}</span>
                     <button
           ...
           <div className="relative">
             <div className="bg-zinc-950 rounded-xl p-4 border border-zinc-800 shadow-inner">
               <p className="text-xs text-zinc-400 text-center mb-4 font-sans">পিডিএফ রিপোর্টে বিস্তারিত ডাটা অন্তর্ভুক্ত করা হবে</p>
             </div>
             ...
             <div style={{ position: "absolute", top: "-9999px", left: "-9999px" }}>
               <div ref={contentRef} className="p-8 bg-white text-black w-[800px] font-sans">
                 <h1 className="text-3xl font-bold border-b border-gray-300 pb-2 mb-4">মেস রিপোর্ট: {reportTitle}</h1>
                 <p className="text-gray-500 mb-6">Generated on: {new Date().toLocaleString()}</p>
                 
                 <h2 className="text-xl font-bold mb-2">সদস্য তালিকা ({members.length})</h2>
                   ...
                   <tbody>
                     {members.map(m => (
                       <tr key={m.id}>
                         ...
                         <td className="border border-gray-300 p-2 font-mono">{m.id}</td>
                         <td className="border border-gray-300 p-2 font-mono">{(deposits[m.id] || 0) + (expenses.filter(e => e.memberId === m.id).reduce((sum, e) => sum + e.amount, 0))}</td>
                       </tr>
                     ))}
                   </tbody>
                 </table>
                 ...
                 <table className="w-full text-left border-collapse border border-gray-300 mb-6 text-sm">
                   ...
                   <tbody>
                     {expenses.map((e, idx) => {
                       const memberName = members.find(m => m.id === e.memberId)?.name || "Unknown";
                       return (
                         <tr key={idx}>
                           <td className="border border-gray-300 p-2">{e.date}</td>
                           <td className="border border-gray-300 p-2">{memberName}</td>
                   ...
                   <tbody>
                     {utilities.map((u, idx) => (
                       <tr key={idx}>
                         ...
                         <td className="border border-gray-300 p-2">{u.amount}</td>
                       </tr>
                     ))}
                   </tbody>
                 </table>
